...
const TodaysTask = () => {
    ...
    const fetchUpcomingNotes = async () => {
        try {
            const res = await axios.post('http://localhost:8080/api/v1/todaysTask', {
                user: user._id
            });
            if (res.data.success) {
        ...
        <>
            <Layout>
                {notes && notes.length > 0 ? (
                        ...
                        {notes.map((note, index) => (
                            ...
                            >
                                ...
                                <div className='mt-auto flex  justify-center space-x-2 mb-2'>
                                    {/* <button type="button" onClick={()=>handleDelete(note._id)} className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 justify-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Task Completed</button> */}
                    ...
                    <p className='text-4xl font-bold flex justify-center'>No Task For Today</p>
                )}
            </Layout>
        </>
    )
